...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for AlibabaCloud polardb").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-upjet-alibabacloud-polardb"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-upjet-alibabacloud-polardb",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(controller.Setup_polardb(mgr, o), "Cannot setup AlibabaCloud polardb controllers")
