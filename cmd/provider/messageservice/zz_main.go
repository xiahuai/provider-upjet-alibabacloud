...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for AlibabaCloud messageservice").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-upjet-alibabacloud-messageservice"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-upjet-alibabacloud-messageservice",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(controller.Setup_messageservice(mgr, o), "Cannot setup AlibabaCloud messageservice controllers")
