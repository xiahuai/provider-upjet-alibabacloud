...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for AlibabaCloud privatelink").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-upjet-alibabacloud-privatelink"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-upjet-alibabacloud-privatelink",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(controller.Setup_privatelink(mgr, o), "Cannot setup AlibabaCloud privatelink controllers")
