...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for AlibabaCloud cdn").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-upjet-alibabacloud-cdn"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-upjet-alibabacloud-cdn",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(controller.Setup_cdn(mgr, o), "Cannot setup AlibabaCloud cdn controllers")
