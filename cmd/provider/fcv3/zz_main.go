...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for AlibabaCloud fcv3").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-upjet-alibabacloud-fcv3"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-upjet-alibabacloud-fcv3",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(controller.Setup_fcv3(mgr, o), "Cannot setup AlibabaCloud fcv3 controllers")
